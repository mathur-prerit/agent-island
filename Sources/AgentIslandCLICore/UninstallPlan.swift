...
public enum UninstallAction: Equatable {
    ...
    case removeDirectory(path: String)              // `~/.agent-island`
...
public enum UninstallPlan {
    ...
    /// login item, then the binaries, the data dir, and finally the `.app`.
    public static func plan(_ paths: InstallPaths) -> [UninstallAction] {
        var actions: [UninstallAction] = [
            .reverseHooks(settingsPath: paths.settingsPath),
            .unregisterLoginItem,
        ]
        actions += paths.binaryPaths.map { .removeBinary(path: $0) }
        actions.append(.removeDirectory(path: paths.agentIslandDir))
