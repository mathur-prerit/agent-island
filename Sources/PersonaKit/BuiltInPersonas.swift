/// The personas that ship with v1. Each keeps the core's frozen color semantics and
/// varies only glyph + copy. Glyphs are single-codepoint emoji / geometric symbols that
/// render reliably in a system font (ZWJ sequences like 🏴‍☠️ / 🧑‍🚀 tofu out, so avoid them).
public enum BuiltInPersonas {
    ...
    public static let pirate = Persona(
        ...
        waiting: StateSkin(glyph: "🦜", label: "awaitin' yer orders"),
        waitingPermission: StateSkin(glyph: "🔑", label: "needs yer say-so"),
    ...
    public static let astronaut = Persona(
        ...
        failed: StateSkin(glyph: "💥", label: "mission aborted"))
    ...
    public static let herald = Persona(
        ...
        waitingPermission: StateSkin(glyph: "🗝", label: "thy judgement is sought"),
        finished: StateSkin(glyph: "🏆", label: "the deed is done"),
        failed: StateSkin(glyph: "🔥", label: "the quest hath failed"))
